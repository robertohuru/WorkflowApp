...
Ext.define('WorkflowApp.view.workflow.WorkflowController', {
    ...
    layerCtrl: null,
    wpsServers: [
        {"name": "ILWIS Operations", type: 'ILWIS', "url": "http://130.89.221.193:75/ilwisoperations"},
    ...
    initializeView: function () {
        ...
        Ext.getStore('operations').load({
            ...
            callback: function (records) {
                WorkflowApp.app.wkfCtrl.addWPSprocess(records, 'ILWIS Operations');
                Ext.getStore('operations').load({
                    params: {
                                ...
                                if (countRequiredEmpties > 0) {
                                    ...
                                    Ext.Ajax.request({
                                        ...
                                        success: function (response) {
                                            ...
                                            for (var i = 0; i < result.length; i++) {
                                                ...
                                                if (result[i]['type'] == "geom") {
                                                    ...
                                                    var layer = L.Proj.geoJson(result[i]["result"], {style: style, id: 'ProcessLayers'});
                                                    WorkflowApp.app.wkfCtrl.layers['Process_' + result[i].id] = layer;
                                                    WorkflowApp.app.wkfCtrl.map.fitBounds(layer.getBounds(), {padding: [0, 0]});
                                                                ...
                                                                items: [
                                                                    {
                                                                        ...
                                                                        listeners: {
                                                                            change: function (event, b) {
                                                                                ...
                                                                                if (b == true) {
                                                                                    WorkflowApp.app.wkfCtrl.map.addLayer(WorkflowApp.app.wkfCtrl.layers[name]);
                                                                                } else {
                                                                                    WorkflowApp.app.wkfCtrl.map.removeLayer(WorkflowApp.app.wkfCtrl.layers[name]);
                                                                                }
                                                                            },
                                                                            afterrender: function () {
                                                                                this.setValue(true);
                                                    ...
                                                    for (var j = 0; j < WorkflowApp.app.wkfCtrl.workflows["workflows"][0].operations.length; j++) {
                                                        if (WorkflowApp.app.wkfCtrl.workflows["workflows"][0].operations[j].id === parseInt(result[i]['id'])) {
                                                            url = WorkflowApp.app.wkfCtrl.workflows["workflows"][0].operations[j]["inputs"][1].value;
                                                        }
                                                    }
                                                    var southWest = L.latLng(result[i]['result']['extent']["southwest"][1], result[i]['result']['extent']["southwest"][0]),
                                                            northEast = L.latLng(result[i]['result']['extent']["northeast"][1], result[i]['result']['extent']["northeast"][0]),
                                                            bounds = L.latLngBounds(southWest, northEast);
                                                    WorkflowApp.app.wkfCtrl.map.fitBounds(bounds, {padding: [0, 0]});
                                                    var layer = L.tileLayer.wms(url + '/wms', {
                                                        layers: result[i]['result']['layer'],
                                                        tiled: true,
                                                        format: 'image/png',
                                                        transparent: true,
                                                    ...
                                                    WorkflowApp.app.wkfCtrl.layers['Process_' + result[i].id] = layer;
                                                    layer.addTo(WorkflowApp.app.wkfCtrl.map);
                                                    Ext.getCmp("result_control_panel").add(
                                                            {
                                                                xtype: 'panel',
                                                                layout: {
                                        ...
                                        error: function (jqXHR, textStatus, errorThrown) {
                                            Ext.MessageBox.hide();
                                            Ext.Msg.alert('Error', 'There was error in executing the workflow!', Ext.emptyFn);
                                        }
                                    });
                                }
                                Ext.getCmp("jsonPanel").setHtml("<span><pre>" + JSON.stringify(WorkflowApp.app.wkfCtrl.workflows, null, 2) + "</pre></span>");
                            ...
                            c.store.getRootNode().eachChild(function (child) {
                                child.getData().children.forEach(function (child) {
                                    if (child.name == document.getElementById(event.target.id).innerHTML) {
                                        ...
                                        if (elem.getAttribute('id') !== null && elem.getAttribute('id') == "result_map-innerCt") {
                                            if (data.feature.url.indexOf("GetCoverage") !== -1) {

                                                var layer = L.tileLayer.wms(data.feature.url.split("ows?")[0] + 'wms', {
                                                    layers: data.feature.name,
                                                    tiled: true,
                                                    format: 'image/png',
                                                    transparent: true,
                                                    continuousWorld: true
                                                ...
                                                layer.addTo(WorkflowApp.app.wkfCtrl.map);
                                                var southWest = L.latLng(data.feature.properties.min[1], data.feature.properties.min[0]),
                                                        northEast = L.latLng(data.feature.properties.max[1], data.feature.properties.max[0]),
                                                        bounds = L.latLngBounds(southWest, northEast);
                                                WorkflowApp.app.wkfCtrl.map.fitBounds(bounds, {padding: [0, 0]});
                                                ...
                                                Ext.Ajax.request({
                                                    ...
                                                    success: function (response) {
                                                        var layer = L.Proj.geoJson(JSON.parse(response.responseText), {id: 'ProcessLayers'});
                                                        WorkflowApp.app.wkfCtrl.layers[data.feature.title] = layer;
                                                        WorkflowApp.app.wkfCtrl.map.fitBounds(layer.getBounds(), {padding: [0, 0]});
                                                        layer.addTo(WorkflowApp.app.wkfCtrl.map);
                                                        Ext.getCmp("layers_control_panel").add(
